...
test("photo API validates origin, configuration, size, JPEG, and cloud failures", async () => {
  const original = { url: process.env.SUPABASE_URL, key: process.env.SUPABASE_SERVICE_ROLE_KEY, bucket: process.env.SUPABASE_PHOTO_BUCKET, fetch: globalThis.fetch };
  ...
  try {
    ...
    process.env.SUPABASE_PHOTO_BUCKET = "photos";
    ...
    globalThis.fetch = async (input, init) => {
      assert.equal(String(input), "https://project.supabase.co/storage/v1/object/photos/groups/12345678-1234-4234-8234-123456789abc.jpg");
    ...
    for (const [key, value] of [["SUPABASE_URL", original.url], ["SUPABASE_SERVICE_ROLE_KEY", original.key], ["SUPABASE_PHOTO_BUCKET", original.bucket]]) {
