...
export function useCamera(
  settings: Settings,
  onFrame: (poses: Observation[], moved: boolean, now: number) => void,
) {
  ...
  const release = useCallback(() => {
    generation.current++;
    if (resources.current.frame !== undefined)
      cancelAnimationFrame(resources.current.frame);
    resources.current.stream?.getTracks().forEach((t) => {
      t.onended = null;
      t.stop();
    });
  ...
  const stop = useCallback(() => {
    release();
    setStatus("off");
    callbackRef.current([], false, Date.now());
  }, [release]);
  ...
  async function start() {
    ...
    try {
      ...
      const fileset = await FilesetResolver.forVisionTasks(
        process.env.NEXT_PUBLIC_VISION_WASM_URL ||
      ...
      const frame = (time: number) => {
        ...
        try {
          if (
            ...
            time - lastInference >= 80
          ) {
            lastTime = video.currentTime;
            lastInference = time;
            const result = model.detectForVideo(video, time);
            const now = Date.now();
            const { observations, moved } = tracker.update(
              result.landmarks,
              now,
              settingsRef.current,
            );
          ...
          fail(
            "Pose tracking stopped. Press Start camera to reload the detector.",
          );
        }
      };
      setStatus("live");
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not start the camera or download the pose model.",
      );
    }
  }
